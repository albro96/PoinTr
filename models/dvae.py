...
def knn(ref, query, k=4):
    distances, indices, _ = knn_points(query.transpose(1, 2).contiguous(), ref.transpose(1, 2).contiguous(), K=k)
    return distances.transpose(1, 2).contiguous(), indices.transpose(1, 2).contiguous()    
...
class DiscreteVAE(nn.Module):
    ...
    def get_loss(self, ret, gt, config=None, epoch=0, loss_func=None, square=False):
        ...
        if loss_func is None:
            ...
            loss_fine = chamfer_distance(fine, gt, norm=2)[0]/2
            if not square:
