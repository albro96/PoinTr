...
class PCN(nn.Module):
    def __init__(self, config):
        ...
        self.folding_seed = (
            torch.cat([a, b], dim=0).view(1, 2, grid_size**2).cuda()
        )  # 1 2 S
    ...
    def get_loss(self, ret, gt, epoch=0):
        loss_coarse = chamfer_distance(
            ret[0], gt, norm=self.cd_norm, single_directional=True
        )[0]
    ...
    def forward(self, xyz):
        ...
        point_feat = point_feat.reshape(-1, self.number_fine, 3).transpose(
            2, 1
        )  # B 3 N

        seed = self.folding_seed.unsqueeze(2).expand(
            bs, -1, self.number_coarse, -1
        )  # B 2 M S
