...
class SelfAttnBlockApi(nn.Module):
    ...
    def __init__(
        ...
        assert (
            len(block_tokens) > 0 and len(block_tokens) <= 2
        ), f"invalid block_style {block_style}"
        self.block_length = len(block_tokens)
        self.attn = None
        self.local_attn = None
        for block_token in block_tokens:
            assert block_token in [
                "attn",
                "rw_deform",
                "deform",
                "graph",
    ...
    def forward(self, x, pos, idx=None):
        ...
        if self.block_length == 2:
            if self.combine_style == "concat":
                ...
                x = x + self.drop_path3(
                    self.ls3(self.local_attn(self.norm3(x), pos, idx=idx))
                )
...
class CrossAttnBlockApi(nn.Module):
    ...
    def __init__(
        ...
        assert self_attn_combine_style in [
            "concat",
            "onebyone",
        ...
        assert (
            len(self_attn_block_tokens) > 0 and len(self_attn_block_tokens) <= 2
        ), f"invalid self_attn_block_style {self_attn_block_style}"
        ...
        for self_attn_block_token in self_attn_block_tokens:
            assert self_attn_block_token in [
                "attn",
                "rw_deform",
                "deform",
                "graph",
        ...
        assert cross_attn_combine_style in [
            "concat",
            "onebyone",
        ...
        assert (
            len(cross_attn_block_tokens) > 0 and len(cross_attn_block_tokens) <= 2
        ), f"invalid cross_attn_block_style {cross_attn_block_style}"
        ...
        for cross_attn_block_token in cross_attn_block_tokens:
            assert cross_attn_block_token in [
                "attn",
                "deform",
                "graph",
                "deform_graph",
    ...
    def forward(
        ...
        if denoise_length is None:
            ...
            mask[:-denoise_length, -denoise_length:] = 1.0
        ...
        if self.self_attn_block_length == 2:
            if self.self_attn_combine_style == "concat":
                ...
                q = q + self.drop_path1(
                    self.ls1(self.self_attn(self.norm1(q), mask=mask))
                )
                q = q + self.drop_path3(
                    self.ls3(
        ...
        if self.cross_attn_block_length == 2:
            if self.cross_attn_combine_style == "concat":
                ...
                q = q + self.drop_path4(
                    self.ls4(self.cross_attn(self.norm_q(q), self.norm_v(v)))
                )
                q = q + self.drop_path5(
                    self.ls5(
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        act_layer=nn.GELU,
        norm_layer=nn.LayerNorm,
        self_attn_block_style_list=["attn-deform"],
        self_attn_combine_style="concat",
        cross_attn_block_style_list=["attn-deform"],
        cross_attn_combine_style="concat",
        k=10,
        n_group=2,
    ):
...
class PointTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        assert (
            len(self_attn_block_style_list) == len(cross_attn_block_style_list) == depth
        )
...
class SimpleEncoder(nn.Module):
    ...
    def forward(self, xyz, n_group):
        ...
        assert (
            center.size(1) == n_group
        ), f"expect center to be B {n_group} 3, but got shape {center.shape}"
        ...
        neighborhood = neighborhood.view(
            batch_size, n_group, self.group_size, 3
        ).contiguous()
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        assert self.encoder_type in [
            "graph",
            "pn",
        ], f"unexpected encoder_type {self.encoder_type}"
        ...
        if decoder_config.embed_dim == encoder_config.embed_dim:
            ...
            self.mem_link = nn.Linear(
                encoder_config.embed_dim, decoder_config.embed_dim
            )
...
class AdaPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        assert self.decoder_type in [
            "fold",
            "fc",
        ], f"unexpected decoder_type {self.decoder_type}"
    ...
    def get_loss(self, ret, gt, epoch=1):
        ...
        loss_coarse = chamfer_distance(pred_coarse, gt, norm=self.cd_norm, single_directional=True)[0]
    ...
    def forward(self, xyz):
        ...
        if self.training:
            ...
            pred_fine = (
                rebuild_points[:, :-denoise_length].reshape(B, -1, 3).contiguous()
            )
