...
from utils.CRA_PCN.pointnet2_ops_lib.pointnet2_ops.pointnet2_utils import (
...
from utils.CRA_PCN.utils import (
...
class VectorAttention(nn.Module):
    def __init__(self, in_channel=128, dim=64, n_knn=16, attn_hidden_multiplier=4):
        ...
        self.pos_mlp = nn.Sequential(
            nn.Conv2d(3, dim, 1), nn.BatchNorm2d(dim), nn.ReLU(), nn.Conv2d(dim, dim, 1)
        )
...
class CRT(nn.Module):
    def __init__(
        self, dim_in=128, is_inter=True, down_rates=[1, 4, 2], knns=[16, 12, 8]
    ):
        super().__init__()
        ...
        for i in range(self.num_scale):
            self.attn_lists.append(
                VectorAttention(in_channel=dim_in, dim=64, n_knn=knns[i])
            )

        for i in range(self.num_scale - 1):
            self.q_mlp_lists.append(
                MLP_Res(in_dim=128 * 2, hidden_dim=128, out_dim=128)
            )
            self.s_mlp_lists.append(
                MLP_Res(in_dim=128 * 2, hidden_dim=128, out_dim=128)
            )
...
class Encoder(nn.Module):
    def __init__(self, out_dim=512, n_knn=16):
        ...
        self.crt_1 = CRT(
            dim_in=128, is_inter=False, down_rates=[1, 2, 2], knns=[16, 12, 8]
        )
    ...
    def forward(self, partial_cloud):
        ...
        l1_points, _, _ = self.crt_1(
            [l1_xyz, l1_points], [l1_xyz, l1_points], None, None
        )

        l2_xyz, l2_points, _ = self.sa_module_2(l1_xyz, l1_points)
        l2_points_dim128 = self.conv_21(l2_points)
        l2_points_dim128, _, _ = self.crt_2(
            [l2_xyz, l2_points_dim128], [l2_xyz, l2_points_dim128], None, None
        )
...
class UpTransformer(nn.Module):
    ...
    def forward(self, pos1, query, pos2, key):
        ...
        pos_rel = pos1.reshape((b, -1, n, 1)) - grouping_operation(
            pos2, idx_knn
        )  # (B, 3, N, k)
        ...
        agg = einsum(
            "b c i j, b c i j -> b c i", attention, value
        )  # (B, dim, N*up_factor)
...
class SeedGenerator(nn.Module):
    def __init__(
        self, feat_dim=512, seed_dim=128, n_knn=16, factor=2, attn_channel=True
    ):
        super().__init__()
        self.uptrans = UpTransformer(
            in_channel=256,
            out_channel=128,
            dim=64,
            n_knn=n_knn,
        ...
        self.mlp_4 = nn.Sequential(
            nn.Conv1d(seed_dim, 64, 1), nn.ReLU(), nn.Conv1d(64, 3, 1)
        )
    ...
    def forward(self, feat, patch_xyz, patch_feat, partial):
        ...
        x = (
            fps_subsample(
                torch.cat([seed.permute(0, 2, 1).contiguous(), partial], dim=1), 512
            )
            .permute(0, 2, 1)
            .contiguous()
        )  # b, 3, 512
...
class PN(nn.Module):
    ...
    def forward(self, xyz, global_feat):
        ...
        feat4cat = [
            feat,
            torch.max(feat, 2, keepdim=True)[0].repeat(1, 1, n),
            global_feat.repeat(1, 1, n),
        ]
...
class DeConv(nn.Module):
    ...
    def forward(self, xyz, feat):
        ...
        feat_child = self.mlp_res(
            torch.cat([feat_child, self.upper(feat)], dim=1)
        )  # b, 128, n*r
...
class UpBlock(nn.Module):
    def __init__(
        self, feat_dim=512, down_rates=[1, 4, 2], knns=[16, 12, 8], up_factor=4
    ):
        super().__init__()
        self.pn = PN()
        self.inter_crt = CRT(
            dim_in=128, is_inter=True, down_rates=down_rates, knns=knns
        )
        self.intra_crt = CRT(
            dim_in=128, is_inter=False, down_rates=down_rates, knns=knns
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.ub0 = UpBlock(
            feat_dim=512, down_rates=[1, 2, 2], knns=[16, 12, 8], up_factor=1
        )
        self.ub1 = UpBlock(
            feat_dim=512, down_rates=[1, 2, 2], knns=[16, 12, 8], up_factor=4
        )
        self.ub2 = UpBlock(
            feat_dim=512, down_rates=[1, 4, 2], knns=[16, 12, 8], up_factor=8
        )

    def forward(self, global_f, p0, p_sd, f_sd):
        p1, f0, p0_fps_idxs_122, _ = self.ub0(p0, global_f, [p_sd, f_sd], None, None)
        p2, f1, p1_fps_idxs_122, _ = self.ub1(
            p1, global_f, [p0, f0], None, p0_fps_idxs_122
        )
        p3, _, _______________, _ = self.ub2(p2, global_f, [p1, f1], None, None)

        all_pc = [
            p_sd.permute(0, 2, 1).contiguous(),
            p1.permute(0, 2, 1).contiguous(),
            p2.permute(0, 2, 1).contiguous(),
            p3.permute(0, 2, 1).contiguous(),
        ]
        return all_pc
...
class Decoder_sn55(nn.Module):
    def __init__(
        ...
        self.ub2 = UpBlock(
            feat_dim=512, down_rates=[1, 4, 2], knns=[16, 12, 8], up_factor=4
        )
...
class CRAPCN(nn.Module):
    ...
    def get_loss(self, ret, gt, partial, config=None, cd_norm=None):
        """loss function
        Args
            ret: List of predicted point clouds, order in [Pc, P1, P2, P3...]
        """
        if cd_norm is None:
            cd_norm = config.model.cd_norm

        # -------------------pyramid-----------------------#
        Pc, P1, P2, P3 = ret

        if config.model.get("loss_subsample", True):
            gt_3 = gt
            gt_2 = fps_subsample(gt, P2.shape[1])
            gt_1 = fps_subsample(gt_2, P1.shape[1])
            gt_c = fps_subsample(gt_1, Pc.shape[1])

            cdc = chamfer_distance(Pc, gt_c, norm=cd_norm)[0]
            cd1 = chamfer_distance(P1, gt_1, norm=cd_norm)[0]
            cd2 = chamfer_distance(P2, gt_2, norm=cd_norm)[0]
            cd3 = chamfer_distance(P3, gt_3, norm=cd_norm)[0]
        else:
            cdc = chamfer_distance(Pc, gt, norm=cd_norm, single_directional=True)[0]
            cd1 = chamfer_distance(P1, gt, norm=cd_norm, single_directional=True)[0]
            cd2 = chamfer_distance(P2, gt, norm=cd_norm, single_directional=True)[0]
            cd3 = chamfer_distance(P3, gt, norm=cd_norm)[0]

        loss_decoder = cdc + cd1 + cd2 + cd3

        partial_matching = chamfer_distance(
            partial, P1, norm=cd_norm, single_directional=True
        )[
            0
        ]  # cdc  # PM(partial, P1) -- term is not included in the loss, but is used for evaluation

        loss_total = loss_decoder  # * cfg.TRAIN.get("LOSS_WEIGHT", 1e3)
